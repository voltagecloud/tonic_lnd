// This example only fetches and prints the assets available on the node to the standard output similar to
// `tapcli assets list`.
//
// The program accepts three arguments: address, cert file, macaroon file
// The address must start with `https://`!
//
// Example run: `cargo run --features=taprpc --example list_assets <address> [tls.cert] <file.macaroon>`
...
async fn main() {
    ...
    let info = client
        .taproot_assets()
        .list_assets(voltage_tonic_lnd::taprpc::ListAssetRequest::default())
        .await
